...
<head>
    ...
    <meta property="og:image" content="http://treeth-image-test.s3.amazonaws.com/show/0b91e16b-4623-4ff3-9dae-6c6350826f8e.jpg" />
    <meta property="og:description" content="ACGACGACGACGACG" />
...
<div class="layer_mask" id="loginModel">
	...
	<script type="text/javascript">
			...
			var share_url = "https://www.facebook.com/dialog/feed?app_id=884720968219535";
			...
			var twitter_url = "https://twitter.com/intent/tweet?";
				...
				if(compressImgList == "" || compressImgList == undefined || compressImgList == null){
				  ...
				  /* getImageSize({
			        	...
			        	oncomplete:function(){
			        		  var objWidth = this.width;
			                  var objHeight = this.height;
			        		$(".wall .detailImg").attr({"src":compressImgList[0]});
			        	}
			        });*/
				...
				$(".wall .smallPic:eq(0)").css({"opacity":"1","borderColor":"#66afe9"});
				...
				$(otherComments).each(function(){
					...
					if(this.userId == loginId){
						elem.find(".operate").attr({"src":address + "img/delComment.png","data-toggle":"modal","data-target":"#cancelModel"}).addClass("cancelComment").removeClass("operate");
					}else{
						elem.find(".operate").attr("src",address + "img/reply.png");
					}
					/*var regexp = /@\w+\s+/gi;
				...
				$(imgs).each(function(){
					...
					var cover = $('<div class="cover"><img src="'+src+'" /><input type="hidden" value="'+this.userId+'" class="userId"><input type="hidden" value="'+this.id+'" class="modelId"></div>');
		       ...
		       $(".moreImgs").scroll(function(){
			          ...
			          if(nScrollTop + nDivHight >= nScrollHight){
			           	if(true) {
							...
							$.get("/post/cover/list",{postId:modelId,userId:userId,pageNum:pageIndex,pageSize:13},function(data){
			...
			$(".smallPic").hover(function(){
				...
				$(".smallPic").each(function(index){
					if(index != i){
						$(this).css({"opacity":"0.4","borderColor":"#c3c3c3"});
					}else{
						$(this).css({"opacity":"1","borderColor":"#66afe9"});
					}
				});
				var url=this.src;
				var urlCompress = url.split("/");
				if(urlCompress[3] == "compress"){
					urlCompress[3] = "show";
					var showUrl = urlCompress.join("/");
					$(".wall .detailImg").attr("src",showUrl);
				}else{
			...
			$(".userComment").live("mouseenter",function(){
				$(this).find(".operate").show();
				$(this).find(".cancelComment").show();

			});
			$(".userComment").live("mouseleave",function(){
				$(this).find(".operate").hide();
				$(this).find(".cancelComment").hide();
			});
		...
		var val = $("#editModel textarea").val();
		var id = $("#detailModel .modelId").val();
		
		if(val){
			$.post("/post/append/add",{postId:id,content:val,originalLanguage:"0"},function(data){
				if(data){
					...
					$(appendCon).each(function(){
						var obj = this;
						$(".userModel").find(".userName").html(obj.name);
						var elem = $(".userModel")
						.find(".head")
						.attr("src",obj.img)
						.hide()
